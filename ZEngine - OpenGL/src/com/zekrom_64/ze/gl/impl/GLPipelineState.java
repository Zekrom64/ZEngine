...
public class GLPipelineState implements Cloneable {
	...
	public static interface GLPipelineStateModule {
	...
	public static class GLPipelineBlendState implements GLPipelineStateModule {
		...
		public void bind(GLPipelineState previous, GLPipelineState current) {
			...
			if (blendOpColor != previous.blendState.blendOpColor || blendOpAlpha != previous.blendState.blendOpColor)
				GL20.glBlendEquationSeparate(GLValues.getGLBlendOp(blendOpColor), GLValues.getGLBlendOp(blendOpAlpha));
			if (!blendConstColor.equals(previous.blendState.blendConstColor)) GL14.glBlendColor(blendConstColor.x, blendConstColor.y, blendConstColor.z, blendConstColor.w);
		}
	...
	public static class GLPipelineDepthViewportScissorState implements GLPipelineStateModule {
		...
		public void bind(GLPipelineState current) {
			...
			GLCapabilities caps = current.backend.getCapabilities();
			double minDepth = depthMin, maxDepth = depthMax;
			if (caps.glViewportArrayv != 0) {
		...
		public void bind(GLPipelineState previous, GLPipelineState current) {
			// Depth test
			if (depthCompare != previous.depthViewportScissorState.depthCompare) GL11.glDepthFunc(GLValues.getGLCompare(depthCompare));
			if (depthTestEnabled ^ previous.depthViewportScissorState.depthTestEnabled) {
				if (depthTestEnabled) GL11.glEnable(GL11.GL_DEPTH_TEST);
				else GL11.glDisable(GL11.GL_DEPTH_TEST);
			}
			if (depthWriteEnable ^ previous.depthViewportScissorState.depthWriteEnable) GL11.glDepthMask(depthWriteEnable);
			...
			if (minDepth != previous.depthViewportScissorState.depthMin || maxDepth != previous.depthViewportScissorState.depthMax) GL11.glDepthRange(minDepth, maxDepth);
		}
	...
	public static class GLPipelinePolygonState implements GLPipelineStateModule {
		...
		@Override
		public void bind(GLPipelineState previous, GLPipelineState current) {
			if (cullMode != previous.polygonState.cullMode) GL11.glCullFace(GLValues.getGLFace(cullMode));
			if (frontFaceCW != previous.polygonState.frontFaceCW) GL11.glFrontFace(frontFaceCW ? GL11.GL_CW : GL11.GL_CCW);
			if (polygonMode != previous.polygonState.polygonMode) GL11.glPolygonMode(GL11.GL_FRONT_AND_BACK, GLValues.getGLPolygonMode(polygonMode));
			if (lineWidth != previous.polygonState.lineWidth) GL11.glLineWidth(lineWidth);
		}
	...
	public static class GLPipelineGeometryState implements GLPipelineStateModule {
	...
	public static class GLPipelineShaderState implements GLPipelineStateModule {
		...
		@Override
		public void bind(GLPipelineState previous, GLPipelineState current) {
			if (shaderProgram != previous.shaderState.shaderProgram) GL20.glUseProgram(shaderProgram != null ? shaderProgram.programId : 0);
		}
	...
	public static class GLPipelineStencilState implements GLPipelineStateModule {
	...
	public static class GLPipelineTessellationState implements GLPipelineStateModule {
		...
		@Override
		public void bind(GLPipelineState previous, GLPipelineState current) {
			if (pointsPerPatch != previous.tessellationState.pointsPerPatch) GL40.glPatchParameteri(GL40.GL_PATCH_VERTICES, pointsPerPatch);
		}
